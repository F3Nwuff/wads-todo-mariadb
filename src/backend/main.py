    ...
    try:
        ...
        if results:
            ...
            for result in results:
                task_id, user_id, task, is_completed, timestamp = result
                tasks.append({"task_id": task_id, "user_id": user_id, "task": task, "is_completed": is_completed, "timestamp": timestamp})
            return tasks
        else:
...
def update_task(task_id: int, user_id: str, task: TaskBase):
    cursor = connection.cursor()

    cursor.execute("SELECT task, isCompleted, timestamp FROM taskbase WHERE id = %s AND user_id = %s", (task_id, user_id))
    ...
    if current_task:
        ...
        cursor.execute("UPDATE taskbase SET task = %s, isCompleted = %s, timestamp = %s WHERE id = %s AND user_id = %s",
                       (task.task, new_is_completed, current_timestamp, task_id, user_id))
        connection.commit()
        cursor.close()
